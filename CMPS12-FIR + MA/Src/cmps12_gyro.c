...
#define ACCEL_MA_POINTS 10

extern FIRFilter FIR_accely;

static int16_t accelX_buffer[ACCEL_MA_POINTS] = {0};
static int16_t accelY_buffer[ACCEL_MA_POINTS] = {0};
static int16_t accelZ_buffer[ACCEL_MA_POINTS] = {0};
...
//    accelY_buffer[accel_index] = FIR_accely.out;
    accelY_buffer[accel_index] = getAcceleroY();

    accelZ_buffer[accel_index] = getAcceleroZ();

    accel_index = (accel_index + 1) % ACCEL_MA_POINTS;
    if (accel_count < ACCEL_MA_POINTS) accel_count++;
}
